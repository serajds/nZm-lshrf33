...
export async function normalizeAbsoluteUrls(): Promise<{ filesFixed: number; reportsFixed: number }> {
  ...
  const filesFixed = (fileResult as unknown as { rowCount: number | null }).rowCount ?? 0;
  const reportsFixed = (reportResult as unknown as { rowCount: number | null }).rowCount ?? 0;
